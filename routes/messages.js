...
router.route('/:id')
    .get(function(req,res){ // Gambiarra
      mongoose.model('message').findById(req.id).populate('user_id').exec(function(error, message) {
        mongoose.model('user').find({}, function (err, users) {
          mongoose.model('comment').find({message_id : message._id}).populate('user_id').exec(function(error, comments) {

              res.render('messages/show', { 'message': message, 'comments':comments, 'users':users });

              //var o = extend({}, results);
              //extend(o,  results2);
              //console.log(o);
              //res.render('messages/show',o);
            });
      });
     });
...
/*router.route('/:id')
  .get(function(req, res) {
    ...
    var messageQuery = mongoose.model('message').findById(req.id).populate('user_id');
    var resources = {
